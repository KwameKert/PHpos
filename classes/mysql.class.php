...
class MySQL
{
	...
	private $db_dbname  = "pharmacydb";          // database name
	private $db_charset = "";          // optional character set (i.e. utf8)
	private $db_pcon    = false;      // use persistent connection?
/*
private $db_host    = "localhost"; // server name
	private $db_user    = "applygtu_gtucapp";          // user name
	private $db_pass    = 'p@$$w0rd4gtucapp';         
	// password
	private $db_dbname  = "applygtu_application";          // database name
	private $db_charset = "";          // optional character set (i.e. utf8)
	private $db_pcon    = false;      // use persistent connection?
	...
	public function __construct($connect = true, $database = null, $server = null,
		...
		if(isset($_SESSION['EVAL_USER'])){
			$this->sectionID=$_SESSION['EVAL_USER']->sectionid;
			$this->officeID=$_SESSION['EVAL_USER']->office;
		}

	}

/** 
	*Search member to check if member is in database and returns its ID
	*/

	public function memberExists($membershipID=null){

		$query="select * from tblmember where membershipID='".$membershipID."'";
		$rs=$this->Query($query);
		//return $this->Error();
		$exists=$this->RowCount();
		$row=$this->Row();
		//var_dump($row);
		if($exists){
			return $row->memberID;
	...
	public function getMemNameByMemID($membershipID=null){
		...
		//var_dump($row);
		if($exists){
			return $row->title." ".$row->lname.", ".$row->fname;
		} else return "";
	 ...
	 *
	 */
    function gradeScore($exam, $score){
        ...
        elseif($score < 40 && $exam > 0){$return = "F";}
        elseif($score < 40 && $exam < 0){$return = "I";}
        return $return;
    }
	 ...
	 */
	static public function BuildSQLInsert($tableName, $valuesArray,$onduplicate=null) {
		...
		$sql = "INSERT INTO `" . $tableName .
			   "` (" . $columns . ") VALUES (" . $values . ")";
                if(is_array($onduplicate)){
                    $sql.=self::BuildSQLOnDuplicateClause($onduplicate);
                }
		return $sql;
	}
	...
	*/
	 ...
	 *
	 * @param string $tableName The name of the table
	 * @return boolean Returns records on success or FALSE 3on error
	 */
	...
	static public function getStatusDesign($status=null){
		$array=array();
		$array['Active']="<span class='label label-paid arrowed-in-right arrowed-in'>Active</span>";
		$array['Deactivated']="<span class='label label-warning arrowed-in-right arrowed-in'>Deactivated</span>";
		$array['Deleted']="<span class='label label-danger arrowed-in-right arrowed-in'>Deleted</span>";
		return $array[$status];
	}

	public function getUserLevels($memberID){
		$query="SELECT
			church_role.rowID,
			church_role.short_code,
			church_role.tbllevelID,
			tbllevel.access_level,
			church_role_member_rel.groupID
			FROM
			church_role
			INNER JOIN church_role_member_rel ON church_role.rowID = church_role_member_rel.roleID
			inner join tbllevel on tbllevel.levelID=church_role.tbllevelID
			WHERE
			church_role_member_rel.memberID = $memberID AND
			church_role.`status` = 1 AND
			church_role_member_rel.`status` = 1
			";
			//return $query;
		$this->Query($query);
		if($this->RowCount()>0){
		$_SESSION['user_levels']="";
		$_SESSION['managed_groups']="";
		$_SESSION['managed_levels']="";
		//$i=0;
		while(!$this->EndOfSeek()){
				$row=$this->Row();
				$_SESSION['user_levels'].="'".$row->short_code."',";
				$_SESSION['managed_groups'].=$row->groupID.",";
				$_SESSION['managed_levels'].=$row->access_level.",";
			//	$i++;
		}
		$_SESSION['user_levels']=rtrim($_SESSION['user_levels'],",");
		$_SESSION['managed_groups']=rtrim($_SESSION['managed_groups'],",");
		$_SESSION['managed_levels']=rtrim($_SESSION['managed_levels'],",");
		$rs=explode(",",$_SESSION['managed_levels']);
		asort($rs);
		$_SESSION['managed_levels']=implode(",",$rs);
	...
	public function getHighestAccessLevel(){
		if(isset($_SESSION['managed_levels'])){
			$level=explode(",",$_SESSION['managed_levels']);
			$cur_level=$level[0];
			return $cur_level;
		}else return -1;
	}
	public function getQuestionTypeName($type){
		$arr=array("mcq"=>"Multiple Choice - Numeric","mcq-text"=>"Multiple Choice -Text","trof"=>"True or False","radio"=>"Radio","label"=>"Label","mcq-multi"=>"Multiple Choice - Multiselect","txt"=>"Short Answer","essay"=>"Essay Type","radio-grid"=>"Multiple Choice Radio Grid","mcq-multi-reason"=>"Multiple Choice with Reason","trof-conditional"=>"True or False - Conditional","txt-list"=>"User Generated List","radio-grid-text"=>"Radio Grid with Text");
		return $arr[$type];	
	}
	
	static public function jsonToArray($json){
		$arr=new RecursiveIteratorIterator(new RecursiveArrayIterator(json_decode($json, TRUE)),RecursiveIteratorIterator::SELF_FIRST);
		
		return $arr;
	}

	public function generateReturnMessage($id){
	    switch ($id){
            case 1:
            ...
            case 3:
                ...
                break;
            case 4:
                return 'Unexpected control character error please try again.';
                break;
            case 5:
                return ' Email address does not exist please enter a valid email address.';
                break;
            case 6:
                return 'Account creation was successful, kindly check email to activate your account.';
                break;
            case 7:
                return 'Account already exist please reset your password if forgotten or enter another email addres.';
                break;
            case 8:
                return 'Account creation failed, please try again later.';
                break;
            case 9:
                return 'Password reset successfully please check your email for a link to set a new password.';
                break;
            case 10:
                return 'Please enter a different password from your previous.';
                break;
            default:
        ...
        public function getLocationChild($type,$id){
            switch ($type){
                case 'country':
                    $this->Query("select * from tblregion where countryid=$id");
                    $results="";
                    while(!$this->EndOfSeek()){
                        $row=$this->Row();
                        
                        $results.= "<option value='".$row->regionid."'>".$row->regionname."</option>";
                    }
                     
                    return $results;
                    break;
                 case 'region':
                     $this->Query("select * from tbldistrict where regionid=$id");
                     $results="";
                    while(!$this->EndOfSeek()){
                        $row=$this->Row();
                        
                        $results.= "<option value='".$row->districtid."'>".$row->districtname."</option>";
                    }
                     
                    return $results;
                    break;
                case 'district':
                    $this->Query("select * from tblcity where districtid=$id");
                    $results="";
                    while(!$this->EndOfSeek()){
                        $row=$this->Row();
                        
                        $results.= "<option value='".$row->cityid."'>".$row->cityname."</option>";
                    }
                     
                    return $results;
                    break;
                case 'city':
                    $this->Query("select * from tblsuburb where cityid=$id");
                    $results="";
                    while(!$this->EndOfSeek()){
                        $row=$this->Row();
                        
                        $results.= "<option value='".$row->suburbid."'>".$row->suburbname."</option>";
                    }
    ...
    function showSemester($semester){
        ...
        $year1 = $year2-1;
        $sem = substr($semester,-1,2);
        if($sem == "01"){$sem = "First";}else{$sem = "Second";}
        return "$year1/$year2 $sem Semester";
    }

    function showYear($semester){
        $year2 = substr($semester,0,4);
        $year1  = $year2-1;
        return "$year1/$year2";
    }
  ...
  static public function processTableListName($list,$targetArray=array()){
      $pr="";
      if(sizeof($targetArray)>0){   
      $suppm=explode(",",$list);
      
      for($i=0;$i<count($suppm);$i++){
          $pr.=$targetArray[$suppm[$i]].", ";
      }
            return rtrim($pr, ', ');
      }else{
             return "";
    ...
    function checkNavigation($pagesubid){

        if (!isset($_SESSION)) {session_start();}

        $psid = $pagesubid;
        $mycat = $_SESSION['term_UserGroup'];
        $currentUserPrivs = array();

        $this->Query("SELECT usr_links.link_id, usr_links.link_url, usr_links.page_id_sub
        FROM `usr_links` JOIN usr_cat_links ON usr_links.link_id = usr_cat_links.link_id
        WHERE usr_cat_links.cat_id = $mycat AND usr_links.link_url <> '#'");

        $queryResultsCount = $this->RowCount();

        if($queryResultsCount > 0){

            while(!$this->EndOfSeek()){

                $thisqueryRow = $this->Row();
                $currentUserPrivs[] = $thisqueryRow->page_id_sub;

            }

            if(in_array($psid, $currentUserPrivs) === false){

                $valuesArray['user_id'] = MySQL::SQLValue($_SESSION['app_user']['user_id']);
                $valuesArray['ip_address'] = MySQL::SQLValue($_SERVER['REMOTE_ADDR']);
                $valuesArray['page_attempted'] = MySQL::SQLValue($psid);

                $tableName = 'nav_breach_log';

                $sql = MySQL::BuildSQLInsert($tableName,$valuesArray);
                $this->Query($sql);

                unset($_SESSION['term_Username']);
                unset($_SESSION['term_UserGroup']);
                unset($_SESSION['term_User']);
            ...
            $valuesArray['user_id'] = MySQL::SQLValue($_SESSION['term_User']['user_id']);
            $valuesArray['ip_address'] = MySQL::SQLValue($_SERVER['REMOTE_ADDR']);
            $valuesArray['page_attempted'] = MySQL::SQLValue($psid);
...
static public function BuildSQLOnDuplicateClause($whereArray) {
   ...
   /**
    * This is to query multiple items at the same time.
    *  @param SQL - This is the aggregated queries ready for execution
    */
   public function multi_query($sql){
       if(mysqli_multi_query($this->mysql_link, $sql)){
   ...
   public function multi_query_for($sql){
       if(mysqli_multi_query($this->mysql_link, $sql)){
           mysqli_close($this->mysql_link);
           $this->Open();
           return true;
    ...
    public function logActivity($activity, $userid,$type){
    	$ip=$_SERVER['REMOTE_ADDR'];
		 $q="insert into activity_logs(ip,activity,date_time,userID,type) values('$ip','$activity',now(),$userid,'$type')";
		$this->Query($q);
		return true;
	}
	...
	public function getErrors($type=""){
		switch($type){
			case "":
				...
				$action="Try again soon!";
				break;
			case "no-assignment":
				$err="No Course Has Been Assigned to You!";
				$action="See your Coordinator or Head of Department to rectify this!";
		}
		return array("error"=>$err,"action"=>$action);
	}
}
?>